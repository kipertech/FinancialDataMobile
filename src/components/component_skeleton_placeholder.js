...
function SkeletonPlaceholder({ children, backgroundColor = "#E1E9EE", speed = 800, highlightColor = "#F2F8FC" })
{
    ...
    const viewStyle = React.useMemo(() => {return{ backgroundColor, overflow: "hidden" };}, [backgroundColor]);
    const getChildren = React.useCallback((element) => React.Children.map(element, (child, index) =>
    {
        let style;
...
module.exports = { SkeletonPlaceholder };
